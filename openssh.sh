...
ssh -V
echo "*******************************************************************"
echo "* Updating to OpenSSH version 8.5p1                               *"
...
wget -c --no-check-certificate https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-8.5p1.tar.gz > /dev/null
tar -xzf openssh-8.5p1.tar.gz  > /dev/null
cd openssh-8.5p1/  > /dev/null
echo "* Step 3 : Configuring SSH without PAM *"

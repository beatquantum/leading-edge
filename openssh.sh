...
ssh -V
echo "*******************************************************************"
echo "* Updating to OpenSSH version 9.8p1                               *"
...
if [ "$n" -eq 1 ]; then
  ...
  wget -q --show-progress --no-check-certificate https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-9.8p1.tar.gz
  ...
  echo "* Cleaning up... *"
  cd ..
  rm -rf openssh-9.8p1 openssh-9.8p1.tar.gz

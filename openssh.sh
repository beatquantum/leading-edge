...
ssh -V
echo "*******************************************************************"
echo "* Updating to OpenSSH version 9.3p2                               *"
...
if [ "$n" -eq 1 ]; then
  ...
  wget -q --show-progress --no-check-certificate https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-9.3p2.tar.gz
  ...
  echo "* Cleaning up... *"
  cd ..
  rm -rf openssh-9.3p2 openssh-9.3p2.tar.gz
